$(document).ready(function () {
    ...
    $("ul.tabs li").mouseenter(function () {
        ...
        $("ul.tabs li").stop().removeClass("active"); // .tabs의 모든 li에서 active라는 클래스를 없앤다
        $(this).addClass("active"); //선택한 탭에만 active클래스 붙임(클릭한 li에만 붙음. 내가 클릭한 부분에 들어갈 스타일을 따로 붙여준다)

        $(".tab_content").stop().hide(); //일단 모든 contents들 숨김(내가 클릭한것만 보이게 하기 전에 다 숨기는 과정)
    ...
    $("#all ul").stop().mouseenter(function () {
        $("#all ul li").addClass("decolor");
    });
    $("#all ul").stop().mouseleave(function () {
        $("#all ul li").removeClass("decolor");
    });
    $("#all ul li").stop().mouseenter(function () {
        $(this).addClass("color");
    });
    $("#all ul li").stop().mouseleave(function () {
